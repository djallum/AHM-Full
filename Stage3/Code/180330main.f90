! Log:
! 180330 -- Added a parameter to control the maximum cluster size to be sent to be diagonalized; changing the name of MinCouplingLoc to MinConnectLoc
...
!
  ...
  subroutine GetDos( SitePotential, Teff, ClusterSize, Cont, Weight, weakL, weakR, SitesRemoved, SitesIgnored )
    ...
    else if ( ( ClusterSize .eq. 1 ) .and. ( ClusterSize .le. ClusterMax ) ) then
  ...
  subroutine MinConnectLoc( WeakBonds, Bonds, ClusterStage, SitesRemoved, weakL, weakR)
    implicit none
    ...
    if (numCluster .gt. 0) then
       ...
       print*, "Error in MinConnectLoc", numCluster
       stop
    end if
    
    
  end subroutine MinConnectLoc
  ...
  subroutine Full_DoS( SitePotential, Hopping, Bonds, WeakBonds, SitesIgnored )
    ...
    do while ( (size(WeakBonds) .gt. 1) .and. (WeakBonds(1) .ne. 0)  ) ! Stop when WeakBonds is set to have 1 element or have the first
       ...
       CALL MinConnectLoc( WeakBonds, Bonds, Loop1, SitesRemoved, weakL, weakR)
