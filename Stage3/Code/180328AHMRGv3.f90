...
module SHARED_MODULE
  ...
  REAL, parameter :: bond_cutoff = 0.5 !maximum bond strength allowed for a site or cluster to be removed
  ...
  real, parameter :: DoSMax = maxval((/ DELTA/2 + 1, uSite + 1 /))
...
!
  ...
  subroutine GetDos( SitePotential, Teff, ClusterSize, Cont, Weight, weakL, weakR, SitesRemoved, SitesIgnored )
    ...
    real, dimension(:), allocatable :: tempWeight
    ...
    if ( ClusterSize .eq. 1 ) then
       ...
       CALL Bin_Data(Potential, Hold, tempWeight, BinsPot, DroppedPot, PotMax, PotMin)
    ...
    else if ( ( ClusterSize .gt. 1 ) .and. ( ClusterSize .le. 8 ) ) then
  ...
  subroutine Full_DoS( SitePotential, Hopping, Bonds, WeakBonds, SitesIgnored )
    ...
    integer, dimension(:), allocatable :: ClusterCount
    integer numCluster
    real, dimension(:,:), allocatable :: Order
                !---------------------------Initializing-------------------------------------
    ...
    do while ( (size(WeakBonds) .gt. 1) .and. (WeakBonds(1) .ne. 0)  ) ! Stop when WeakBonds is set to have 1 element or have the first
       ...
       if ( ClusterSize .eq. Loop1 ) then                                 ! If the current cluster is small enough then remove it                      
          ...
          CALL Bin_Data( Dos, Cont, Weight, BinsDoS, &                   ! Bin the contributions based on their weights using the Bin_Data routine
               DroppedDos, DoSMax, DoSMin )
    ...
    CALL GetDoS( SitePotential, Teff, ClusterSize &
         , Cont, Weight, weakL, weakR, SitesRemoved, SitesIgnored ) 
    CALL Bin_Data( DoS, Cont, Weight, BinsDoS, DroppedDos, DoSMax, DoSMin )
  ...
  subroutine Bin_Data( HistoData, Data, Weights, BinValue, Dropped, Max, Min )
    implicit none
...
program AndersonHubbard
     ...
     do loop1 = 1,bins
        BinsDoS(loop1) = DoSMin + (DoSMax-DoSMin)*loop1/real(bins)
        BinsPot(loop1) = PotMin + (PotMax-PotMin)*loop1/real(bins)
     end do
     ...
     do Loop1=1,bins
        write(100,400) BinsDoS(Loop1) , DoS(Loop1)/( Sum(DoS) * BinWidth )
     end do
