! Log:
  ...
  ! Created: 171018; Last Edited: 180412
  ...
  ! (1) Set up system of 'dim' sites, each with a site potential stored in 'SitePotential'. The values for the site potentials are selected randomly from a uniform distribution of width 'DELTA' from [-DELTA/2,DELTA/2]. Between each site, a hopping ampitude is defined which governs the overlap between neighbouring orbitals, these are stored in the array 'hopping' where the i'th element is the hopping amplitude from site i to i+1. Initially, the hopping amplitude is the same for each site but is changed during the RG process to capture the information of removed sites. An interaction energy is defined which is the energy cost to having two electrons on the same site. This will be stored in 'uSite' and is the same for each site.
  ! (2) Identify all the weak bonds in the cluster. A bond between sites is defined by abs(t/E_ij) (E_ij is the smallest difference in energy between all possible pairs on two adjacent sites. E_ij is chosen as the smallest of |E_i-E_j|, |(E_i+U)-E_j| and |E_i-(E_j+U)| where E_i/j+U is the upper hubbard orbital for site i/j
  ! (3) The sites between the weak bonds are called clusters which for the purpose of this code are independent from the lattice (closed boundary conditions) and are diagonalized as their own systems.
  ...
  !          Density of States bandwidth: DoSMax - DoSMin
  !          Density of states bins:
