! Log:
! 180413 --- Changed the binning procedure, no more +1
...
module Diag
  ...
  real, parameter :: DosMin = -20, DosMax = 20
...
!************************************************************************************
!************************************************************************************
subroutine num_sites(nsites, fock_states)
	!    %----------------------------------------------------------------------------------------------%
	...
	integer :: i, j, istate, isite  ! counters for loops
        ...
        integer, dimension(:,:), intent(out) :: fock_states                ! array that stores each FS represented in binary (see above)
...
!************************************************************************************
!************************************************************************************
subroutine transformations(PES_up, PES_down, IPES_up, IPES_down, phase_PES_up, &
     phase_PES_down, phase_IPES_up, phase_IPES_down, nsites, nstates, fock_states)
	!  %---------------------------------------------------------------------------------------------------%
...
!************************************************************************************
!************************************************************************************
subroutine matrix_sizes(nsites, msize, mblock)
	!  %------------------------------------------------------------------------------%
	...
	integer :: n_up,n_dn
        integer, intent(in) :: nsites
        integer, dimension(0:nsites,0:nsites), intent(out) :: msize       ! msize(i,j) is size of submatrix with n_up=i and n_dn=j
        integer, dimension(0:nsites,0:nsites), intent(out) :: mblock      ! mblock(i,j) is index in fock_states array of first state with n_up=i,n_dn=j
...
!************************************************************************************
!************************************************************************************
subroutine BuildHSUB(n_up, n_dn, HSUB, neighbours, mblock, msize, fock_states, U, E, t, nsites, nstates)
  implicit none
  integer, intent(in) :: n_up,n_dn
  real, dimension(:,:), intent(out) :: HSUB
  integer, intent(in) :: nsites, nstates
  integer, dimension(0:nsites,0:nsites), intent(in) :: msize       ! msize(i,j) is size of submatrix with n_up=i and n_dn=j
  integer, dimension(0:nsites,0:nsites), intent(in) :: mblock      ! mblock(i,j) is index in fock_states array of first state with n_up=i,n_dn=j
  integer, intent(in), dimension(:,:) :: neighbours
  integer, intent(in), dimension(:,:) :: fock_states       ! array that stores each FS represented in binary (see above)
...
!************************************************************************************
!************************************************************************************ 
subroutine solve_hamiltonian1(E, U, mu, t, neighbours, mblock, msize, fock_states, e_ground, nsites, nstates)
  ...
  real, allocatable :: HSUB(:,:), VSUB(:,:), WSUB(:)
  integer :: i

  do n_up = 0,nsites             ! loop over all submatrices
     do n_dn = 0,nsites             ! loop over all submatrices
        allocate(HSUB(msize(n_up,n_dn),msize(n_up,n_dn)),VSUB(msize(n_up,n_dn),msize(n_up,n_dn)))
        allocate(WSUB(msize(n_up,n_dn)))
        call BuildHSUB(n_up, n_dn, HSUB, neighbours, mblock, msize, fock_states, U, E, t, nsites, nstates)
        call ssyevr_lapack1(msize(n_up,n_dn),HSUB,WSUB,VSUB)
        e_ground(n_up,n_dn) = WSUB(1) - mu*(n_up+n_dn)
        deallocate(HSUB,VSUB,WSUB)
     end do
  end do
...
!************************************************************************************
!************************************************************************************
subroutine solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates, &
  ...
  integer :: i
        ...
        allocate(HSUB(msize(g_up,g_dn),msize(g_up,g_dn)),VSUB(msize(g_up,g_dn),msize(g_up,g_dn)))
        allocate(WSUB(msize(g_up,g_dn)))
        ...
        call BuildHSUB(g_up, g_dn, HSUB, neighbours, mblock, msize, fock_states, U, E, t, nsites, nstates)
        ...
        do i=1,msize(g_up,g_dn)
           eigenvectors(i+mblock(g_up,g_dn)-1)%comp(1:msize(g_up,g_dn)) = VSUB(1:msize(g_up,g_dn),i)  ! eigenvectors
        end do
        deallocate(HSUB,VSUB, WSUB)
...
!************************************************************************************
!************************************************************************************
subroutine DiagCluster(nsites,nstates,Cont,Weight,E,mu,U,t)
  implicit none
  integer, intent(in) :: nsites, nstates
  real, intent(in) :: E(nsites), mu, U, t
  real, dimension(nsites*2*nstates), intent(out) :: Cont, Weight
  ...
  TYPE(cell), allocatable, dimension(:) :: eigenvectors       ! the many body eigenvectors (MBE) only coefficients of basis states with same n_up,n_dn as it  
  ...
  call num_sites(nsites, fock_states)
  call transformations(PES_up, PES_down, IPES_up, IPES_down, phase_PES_up, &
     phase_PES_down, phase_IPES_up, phase_IPES_down, nsites, nstates, fock_states)
  call make_neighbours(nsites, neighbours)
  CALL matrix_sizes(nsites, msize, mblock)
  do i=1,2
     !print*, neighbours(:,i)
  end do  
  ...
  call solve_hamiltonian1(E, U, mu, t, neighbours, mblock, msize, fock_states, e_ground, nsites, nstates)  ! solve for the lowest grand potential of each hamiltonian sub-matrix
  ...
  call solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates,&
       g_up,g_dn,eigenvectors,grand_potential)

  
  !---------Solve for eigenvectors and eigenvalues all sub hamiltonians with +/- 1 electron as MBG--------------------
  if (g_up /= 0) call solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates,&
       g_up-1,g_dn,eigenvectors,grand_potential)
  if (g_dn /= 0) call solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates,&
       g_up,g_dn-1,eigenvectors,grand_potential)
  
  if (g_up /= nsites) call solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates,&
       g_up+1,g_dn,eigenvectors,grand_potential)
  if (g_dn /= nsites) call solve_hamiltonian2(E,U,mu,t,neighbours,mblock,msize,fock_states,nsites,nstates,&
       g_up,g_dn+1,eigenvectors,grand_potential)
...
!************************************************************************************
   ...
   program TEST
     use Diag
     implicit none
     integer, parameter :: nsites = 5
     integer, parameter :: nstates = 4**nsites
     integer, parameter :: Cl = 100000
     real, allocatable, dimension(:) :: Cont, Weight
     real :: start, finish, TIME
     real, parameter :: U = 6, mu = U/2, t = -1, W = 10
     real E(nsites)
     integer i,j,k
     integer, parameter :: seed =345
     real :: DOS(bins)
     real :: BinVs(bins)
     real :: BinWidth
     ...
     do i=1,bins
        BinVs(i) = DosMin + (DosMax-DosMin)*i/real(bins)
     end do
     DOS = 0.0

     allocate(Cont(2*nsites*nstates))
     allocate(Weight(2*nsites*nstates))
     ...
     Weight = 0.0  
        ...
        call DiagCluster(nsites,nstates,Cont,Weight,E,mu,U,t)
        
        
     do i = 1,Size(Cont)
	if ( Weight(i) .eq. 0 ) CYCLE
        if ( (Cont(i) .ge. DosMax) .or. (Cont(i) .lt. DosMin) ) CYCLE
        k = Floor(bins*(Cont(i) - DosMin)/(DosMax-DosMin))
        if ( k .gt. bins ) then
           print*, Cont(i)
           print*, "Stopping"
           stop
        ...
        Dos(k) = Dos(k) + Weight(i)
     end do

      
     end do
     
     open (unit = 100, file = "DOS_5_t.text" ,status ='unknown' )

     !Comments for the file above, says whats in the file and which parameters go with the data
     write(100,*) "#This file contains the density of states for an ensemble. First column is", &
          " right edge of bin, fraction of contributions per bin"
     write(100,*) "#Sites in clusters", nsites
     write(100,*) "#parameters found in D*Site number*.log"
     
     BinWidth = BinVs(2) - BinVs(1)
     do i = 1,bins
        write(100,400) BinVs(i), DOS(i)/( Sum(Dos) * BinWidth) 
     end do
     CALL CPU_TIME(finish)
     TIME = finish-start
     Print*, " this is an ensemble of clusters of this many sites: ", nsites
     print '("Time = ", I1.1," days,", I2.1," hours,", I2.1," minutes, and ", I2.1," seconds.")', &
          int(TIME/real(86400)), mod(int(TIME/real(3600)),24), mod(int(TIME/real(60)),60), mod(int(TIME),60)
     print*, ""
     print*, "Disorder Strength: ", W
     print*, "Interaction Strength: ", U
     print*, "Ensemble Size: ", Cl
     print*, "Start Time (s): ", Start, "End Time (s): ", Finish
     print*, "Difference (s): ", Finish - Start
400  format( g12.5, g12.5) 
     close(100)
   end program TEST
